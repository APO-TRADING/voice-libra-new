...
// ──────────────────────────────────────────────────────────────────────
// 4. PHONEME STRETCH FACTOR — v1.0.5
//
//    English phonemes generated by the en-us espeak voice are rendered
//    ~25% faster than the surrounding Italian by Piper VITS (the model
//    is trained on italian prosody and "eats" foreign sequences).
//    We compensate by adding a stretch="N" attribute on every <phoneme>
//    tag — the C++ parser reads it and duplicates IPA vowel characters
//    proportionally to the factor (see stretchIpaVowels in phonemize_jni
//    .cpp for the algorithm).
//
//    Hardcoded to 1.25 for the audiobook use-case (product owner spec).
//    Sanity-clamped to [1.0, 3.0] on the C++ side too.
// ──────────────────────────────────────────────────────────────────────
...
export async function wrapForeignWords(text: string, srcLang: string): Promise<string> {
  ...
  for (const { start, end, token } of matches) {
    ...
    if (ipa) {
      ...
      // v1.0.5: include stretch="N" so the C++ parser knows to slow
      // down (duplicate vowels in) the IPA sequence proportionally,
      // compensating Piper's italian-trained duration predictor.
      out +=
        `<phoneme alphabet="espeak" ph="${escapeXmlAttribute(ipa)}" stretch="${STRETCH_FACTOR}">` +
        escapeXmlText(token) +
        `</phoneme>`;
      wrappedCount += 1;
    } else {
  ...
  // eslint-disable-next-line no-console
  console.log(`[JIT.wrap] wrapped=${wrappedCount} fallback=${fallbackCount} stretch=${STRETCH_FACTOR} out_len=${out.length} (vs in_len=${text.length})`);
